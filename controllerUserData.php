...
if(isset($_POST['signup'])){
   ...
   //if login now button click
    ...
    if(isset($_POST['get_cards'])) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            if(mysqli_num_rows($user_res) == 1) {
                ...
                if($fetch['status'] == 'pending') $recommend_first = true;
            }
            if($showCard) {
                $noCards = false;
                $id = $row['uid'];
                ...
                if($recommend_first) array_unshift($cards, $card);
                else array_push($cards, $card);
            }
        }
    ...
    if(isset($_POST['advanced_search'])) {
        $gender = $_POST['gender']; //default will be as before
        $matching_hobbies = $_POST['matchingHobbies'];
